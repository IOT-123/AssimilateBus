void on_bus_received(byte slave_address, byte prop_index, Role role, char name[16], char value[16]){
	...
	// collect the property data for one cycle to complete the profile needed to send deviceInfo to Crouton
	...
	// publish line-chart properties if configured in metadata
	if (get_json_is_series(slave_address, prop_index)){
	...
	// publish custom card properties 
	byte dto_idx = get_prop_dto_idx(slave_address, prop_index);
	if (_dto_props[dto_idx].has_custom_card)
	{
		_debug.out_fla(F("-------------------------------------------"), true, 2);
...
void on_bus_complete(){
	...
	_assimilate_bus.get_properties(on_bus_received, on_bus_complete); // starts another request cycle on the I2C bus
	mqtt_create_subscriptions(); // inbox (Crouton convention) subscriptions
}
