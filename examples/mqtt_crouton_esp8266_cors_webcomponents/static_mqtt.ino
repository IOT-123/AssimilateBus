...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
	...
	if (strcmp("toggle", card_type) == 0) {
		...
		i2c_set_and_get(address, 2, slave_payload.c_str());
		return;
	}
	if (strcmp("input", card_type) == 0) {
		i2c_set_and_get(address, 2, root["value"]);// limit to 16?
		return;
	}
	...
	if (strcmp("button", card_type) == 0) {
		if (root["value"])
		{
			i2c_set_and_get(address, 2, "1");
		}
		else {
			i2c_set_and_get(address, 2, "0");
		}
		return;
	}
} // mqtt_callback()
...
{
	_assimilate_bus.send_to_actor(address, code, param);
	// get slave based on address & request an update of the properties on the slave
	_assimilate_bus.get_property(_assimilate_bus.get_slave_by_address(address), on_bus_received);
}
	...
	for (int i = 0; i < _dto_props_index; i++)
	{
		if (strcmp("", _dto_props[i].name) != 0) { // if assigned
			if (_dto_props[i].role == ACTOR) { // only actors
				...
				_debug.out_str(endpoint_name, true, 2);
