...
public:
  ...
  struct Slave
  {
	...
	void get_properties(NameValueCallback nvCallback, SlavesProcessedCallback spCallback);
	void get_property(Slave slave, NameValueCallback nvCallback);
	...
	Slave get_slave_by_address(byte address);
private:
